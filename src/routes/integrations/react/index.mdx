...
Si vous n'avez pas encore d'appli Qwik, vous devez [d'abord en créer une](/docs/(qwik)/getting-started/index.mdx), puis suivre les instructions et exécuter la commande `add react` pour votre appli.
...
> **Remarque :** Vous NE pouvez PAS utiliser les composants React dans Qwik sans les convertir d'abord, en utilisant `qwikify$()`. Même si [les composants React et Qwik se ressemblent](/docs/(qwik)/cheat/qwik-react/index.mdx), ils sont fondamentalement très différents.
