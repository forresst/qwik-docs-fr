...
## N'enregistrez pas les événements en avance avec `useBrowserVisibleTask$()`

> ⚠ Utilisez `useBrowserVisibleTask$()` avec précaution ⚠
...
Cela exige également que vous, le développeur, restiez plus attentif à la relation entre le serveur et le client (il y a une raison pour laquelle vous avez obtenu `use >BrowserVisible< Task` après tout).
