...
- Les outils ne savent pas comment découper la map de traduction, pour cette raison, la map de traduction entière doit être chargée par avance au démarrage de l'application. C'est un problème parce que cela annule l'effort que Qwik a mis dans la décomposition et le chargement différé de votre base de code. En outre, parce que les maps de traduction ne sont pas divisées, le navigateur va télécharger des traductions inutiles. Par exemple, les traductions pour les composants statiques qui ne seront jamais rendus sur le client. 
