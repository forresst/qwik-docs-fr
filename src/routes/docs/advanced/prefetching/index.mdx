...
La prélecture est un moyen pour les applications de commencer à télécharger des modules en tâche de fond, avant que l'utilisateur n'ait réellement besoin de ces modules. La solution idéale est de ne précharger que la plus petite quantité de code qui est hautement susceptible d'être exécutée à partir de l'interaction de l'utilisateur, tout en omettant tout JavaScript qui ne pourrait pas être utilisé.

Qwik excelle dans le téléchargement et l'exécution d'une quantité minimale de JavaScript. Et puisque Qwik est capable de comprendre comment les composants individuels sont utilisés (et ce qui n'est pas utilisé), il peut aussi mieux décider quels bundles doivent être préchargés.
...
Pour notre exemple « Ajouter au panier », l'Optimiseur ne collectera uniquement que les symboles pour l'écouteur d'événement du clic et le moteur de rendu du widget d'ajout au panier. Il n'aura cependant pas à télécharger, hydrater et re-rendre les autres parties de l'application, puisqu'il n'est même pas possible de re-rendre les autres sections de la page.
...
**QUESTION** : _Le chargement différé sur les événements utilisateur est lent car l'utilisateur doit attendre le téléchargement du code ?_
...
**QUESTION** : _La prélecture du code n'entraîne-t-elle pas le même comportement que les frameworks existants qui téléchargent et exécutent tout le code par avance ?_

Non, pour plusieurs raisons :

- Les frameworks existants doivent télécharger et exécuter tout le code ([hydratation](../../../docs/concepts/resumable/index.mdx)) avant que l'application puisse être interactive. Typiquement le téléchargement du code représente un coût en temps plus petit que l'exécution du code.
- La prélecture de code de Qwik télécharge seulement le code mais ne l'exécute pas. Par conséquent, même si Qwik récupère la même quantité de code que les frameworks existants, le gain de temps est significatif.
- Qwik pré lit seulement le code qui est nécessaire pour la page actuelle. Qwik évite de télécharger le code associé aux composants qui sont statiques. Dans le pire des cas, Qwik récupère la même quantité de code que les frameworks existants dans le cas le plus favorable. Dans la plupart des cas, Qwik récupère une petite portion de code comparé aux frameworks existants.- Prefetching of code can happen on other threads than the main thread. Many browsers can even pre-parse the AST of the code off the main thread.
...
- Qwik peut décomposer l'application en plusieurs petits morceaux, et ces morceaux peuvent être téléchargés selon l'ordre de probabilité que l'utilisateur interagisse avec eux. Les frameworks existants ont des difficultés à décomposer les applications en petits morceaux, et il n'y a pas de moyen facile de hiérarchiser l'ordre de téléchargement des morceaux car l'hydratation nécessite un seul point d'entrée "principal" dans l'application.

**QUESTION** : _Qui est responsable de la détermination de la prélecture du code ?_

Qwik peut générer automatiquement les instructions de la prélecture dans le cadre du rendu SSR. En exécutant l'application, Qwik a une connaissance d'exécution des composants visibles, des événements que les utilisateurs peuvent déclencher et du code qui devra être téléchargé. La conséquence est que la prélecture est un ensemble idéal de fichiers pour cette page. Aucune action de la part des développeurs n'est requise autre que de configurer le `renderToStream()` avec la stratégie de la prélecture.
