...
Qwik est conçu pour le chargement différé de votre application. Pour réaliser le chargement différé, Qwik nécessite un petit morceau de JavaScript à charger au début qui sait comment télécharger le reste de l'application en fonction des besoins. Nous appelons ce JavaScript Qwikloader.
...
1. Le navigateur télécharge le HTML et exécute le script Qwikloader intégré. Le Qwikloader met en place des écouteurs globaux pour tous les événements du navigateur.
...
Sans les informations ci-dessus, Qwik serait obligé de télécharger le template du composant et de l'exécuter pour que l'emplacement et la closure de l'écouteur puissent être récupérés. Ce processus est connu sous le nom d'hydratation, et Qwik essaie explicitement d'éviter l'hydratation.
