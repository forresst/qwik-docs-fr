...
- **Quand :** avant le rendu et les ressources
- **Nombre de fois :** une seule fois par instance de composant
- **Platforme :** navigateur uniquement

`useClientMount$()` enregistre un hook monté sur le client qui s'exécute uniquement sur le navigateur lorsque le composant est monté pour la première fois.
...
export const Cmp = component$(() => {
  ...
  useClientMount$(() => {
    // Ce code ne sera exécuté qu'une seule fois UNIQUEMENT dans le navigateur, lorsque le composant sera monté.
...
Pour tous les composants qui ont été montés pendant le SSR (dans le serveur), la fonction `useClientEffect$()` s'exécutera par avance, c'est-à-dire sans interaction de l'utilisateur, mais cette exécution par avance (eagerness) peut être configurée !
