...
```
  useTask$ ------> RENDU -----> useBrowserVisibleTask$
                                |
...
> **Notez** que parce que le composant a été monté sur le serveur, **seulement useBrowserVisibleTask$() s'exécute dans le navigateur**. Cela est dû au fait que le navigateur poursuit le même cycle de vie, qui a été interrompu sur le serveur juste après le rendu et repris dans le navigateur.
...
> Notez que `useTask$()` s'exécute **AVANT** le rendu réel et dans le serveur, donc si vous devez faire une manipulation du DOM, utilisez `useBrowserVisibleTask$()` à la place, qui s'exécute sur le client après le rendu.
...
- Calculer l'état dérivé à partir de l'état traqué (sans provoquer un nouveau rendu potentiel comme dans `useBrowserVisibleTask$`)
...
`useBrowserVisibleTask$()` enregistre un hook à exécuter lorsque le composant devient visible dans le viewport, il s'exécutera au moins une fois dans le navigateur, et il peut être réactif et se ré-exécuter lorsqu'un [signal ou store](/docs/components/state/index.mdx) **traqué** change, comme ceci :
...
Sous le capot, `useBrowserVisibleTask$()` crée un écouteur de DOM qui utilise [Intersection Observer](https://developer.mozilla.org/fr/docs/Web/API/Intersection_Observer_API) pour détecter quand le composant devient visible dans le viewport.

Cependant, cette `strategy` peut être modifiée, et par exemple, être exécutée lorsque le `document` est prêt.

```tsx
useBrowserVisibleTask$(() => console.log('s\'exécute dans le navigateur'), {
...
### Quand utiliser `useBrowserVisibleTask$()`

> **REMARQUE :** N'abusez pas de `useBrowserVisibleTask$()` lorsque la même logique peut être réalisée en utilisant `useTask$()` ou d'autres moyens. Posez-vous la question : Ce code doit-il vraiment être exécuté au début dans le navigateur ? Si la réponse est non, `useBrowserVisibleTask$()` n'est probablement pas la bonne réponse.

Comme son nom l'indique, `useBrowserVisibleTask$()` est utile lorsque vous devez exécuter du code lorsque le composant devient visible dans le viewport. Ceci est utile pour :
