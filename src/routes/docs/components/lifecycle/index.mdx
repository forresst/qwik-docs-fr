...
> **REMARQUE :** N'abusez pas de `useBrowserVisibleTask$()` lorsque la même logique peut être réalisée en utilisant `useTask$()` ou d'autres moyens. Posez-vous la question : Ce code doit-il vraiment être exécuté au début dans le navigateur ? Si la réponse est non, `useBrowserVisibleTask$()` n'est probablement pas la bonne solution.
