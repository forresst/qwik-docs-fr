...
export default component$(() => {
  ...
  return (
    <div>
      ...
      <div>
        La boîte rouge ci-dessus a une hauteur de {height.value} pixels et une largeur de {width.value} pixels.
      </div>
    </div>
  );
});
...
- [`useStore(initialState)`](../state/index.mdx#usestore) - crée un objet réactif qui peut être utilisé pour stocker l'état
- [`createContextId(contentName)`](../context/index.mdx#createcontext) - crée une référence de contexte
...
- [`useTask$()`](../lifecycle/index.mdx#usetask) - définit un callback qui sera appelé avant le rendu et/ou lorsqu'un store surveillé est modifié
- [`useResource$()`](../resource/index.mdx) - crée une ressource pour charger des données de manière asynchrone
- [`useBrowserVisibleTask$()`](../lifecycle/index.mdx#usebrowservisibletask) - définit un callback qui sera appelé après le rendu, uniquement dans le client (navigateur)
