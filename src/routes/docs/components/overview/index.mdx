...
- Un proxie de lecture pendant l'exécution de la méthode OnRender permet à Qwik de savoir que la méthode OnRender dépend d'une propriété donnée. Une lecture crée un abonnement sur cette propriété. Dans notre [exemple](../../concepts/reactivity/#counter-example), OnRender lit `{store.count}`, ce qui crée un abonnement qui indique à Qwik que chaque fois que `store.count` change, le composant doit être invalidé.
...
Alors que se passe-t-il lorsque le composant `Parent` est rendu et que le composant `Child` n'a pas encore été téléchargé ? D'abord, le composant `Parent` rend son JSX comme ceci.
...
- `useStore(initialState)` - crée un store réactif
- `useRef$(initialState)` -crée une ref
- `createContext(contentName)` - crée un contexte
- `useContextProvider$()` - crée un fournisseur de contexte
- `useContext$()` - utilise un contexte

### Styles

- `useStylesScoped$()` - crée un style scopé
- `useStyles$()` - crée un style global

### Événements

- `useOn()` - crée un écouteur d'événement
- `useOnWindow()` - crée un écouteur d'événements de window
- `useOnDocument()` - crée un écouteur d'événement de document

### Cycles de vie

- `useMount$()` - crée un post rendu
- `useServerMount$()` - crée un post rendu
- `useWatch$()` -crée une surveillance
- `useClientEffect$()` - crée un post rendu
- `useResource$()` - crée une ressource
...
- `<Slot>` - crée un slot
- `<SSRStreamBlock>` - crée un bloc de stream
- `<SSRStream>` - crée un stream
- `<Fragment>` - crée un fragment
