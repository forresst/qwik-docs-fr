...
- **Asynchrone** : cela signifie que la fonction `render` de Qwik est consciente qu'elle peut avoir besoin de télécharger des composants enfants, et donc l'opération de rendu est asynchrone.
...
Qwik charge les composants de manière différée, selon les besoins. Pour minimiser le nombre de composants à télécharger, Qwik ne descend dans les composants enfants que si les props du composant ont changé.
...
La nature asynchrone de `render()` signifie que les utilisateurs peuvent voir un rendu intermédiaire de l'interface utilisateur pendant le téléchargement des composants. Voir un état intermédiaire n'est pas souhaitable, par conséquent, Qwik mettra en mémoire toutes les mises à jour du DOM et ne purgera les opérations du DOM que lorsque tous les composants auront été téléchargés et leurs fonctions JSX exécutées. Ainsi, l'interface utilisateur sera mise à jour comme une opération atomique, et l'utilisateur ne verra pas les étapes intermédiaires.
