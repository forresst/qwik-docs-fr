...
Le callback passé à [`useResource$()`](/docs/components/resource/index.mdx) s'exécute juste après que les callbacks `useMount$()` et [`useWatch$()`](/docs/components/lifecycle/index.mdx#usewatch) se soient terminés. Veuillez vous référer à la section [Cycle de vie](../lifecycle/index.mdx) pour plus de détails.
  ...
  return (
    <div>
      ...
      <Resource
        ...
        onResolved={(ageGuess) => {
          return <div>
            {store.name && <>{ageGuess.name} {ageGuess.age} years</>}
          </div>;
        }}
      />
    </div>
    ...
    <div>
      ...
      <div>
        ...
        {!ageResource.loading && store.name && <div>{ageResource.promise.then(ageGuess => (
          <div>{ageGuess.name} {ageGuess.age} years</div>
        ))}</div>}
      </div>
    </div>
  );
