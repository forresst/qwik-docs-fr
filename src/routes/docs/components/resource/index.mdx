...
Le callback passé à [`useResource$()`](/docs/components/resource/index.mdx) s'exécute juste après que les callbacks `useMount$()` et [`useTask$()`](/docs/components/lifecycle/index.mdx#usewatch) se soient terminés. Veuillez vous référer à la section [Cycle de vie](../lifecycle/index.mdx) pour plus de détails.
