...
Notez que `<Resource>` n'est pas nécessaire pour utiliser `useResource$()`, c'est juste un moyen pratique de faire le rendu de l'état de la ressource.

### Exemple d'utilisation de `useResource$()` avec `<Resource>`
...
> **REMARQUE**
> Nous recommandons fortement d'utiliser `<Resource/>` chaque fois que c'est possible, car ce composant évitera plus efficacement les re-rendus et il est également optimisé pour SSR.
