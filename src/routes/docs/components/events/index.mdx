...
Remarquez que `onClick$` se termine par `$`. C'est une indication à la fois pour l'Optimiseur Qwik et le développeur qu'une transformation spéciale a lieu à cet endroit. La présence du suffixe `$` implique ici une zone de chargement différé. Le code associé au gestionnaire de `click` ne sera pas téléchargé jusqu'à ce que l'utilisateur déclenche l'évènement `click`. Voir les [règles de l'Optimiseur](../../advanced/optimizer/index.mdx) pour plus de détails.
