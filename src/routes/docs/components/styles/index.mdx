...
Pour réduire le temps de blocage de la première représentation, Vite peut [mettre en ligne les styles critiques](https://vitejs.dev/guide/features.html#disabling-css-injection-into-the-page) pour la vue courante. Cela signifie que le navigateur n'aura pas besoin de charger séparément les fichiers `*.css` référencés. Pour dire à Vite de mettre les styles en ligne, il suffit d'ajouter le paramètre de requête `?inline` à l'importation.
...
Ceci est réalisé en générant une classe unique pour chaque composant et en insérant cette classe unique dans la feuille de style. Veuillez noter que vous devez ajouter `?inline` à votre importation de styles.
