...
Non. En production, Qwik utilise beaucoup d'informations générées pendant le SSR (Rendu côté serveur) pour commencer la [prélecture](../../qwikcity/prefetching/overview/index.mdx) uniquement des morceaux d'interactivité disponibles sur la page courante dès que possible (ASAP). De cette façon, quand l'utilisateur clique ou interagit, le JS est déjà chargé.
...
De plus, la [prélecture](../../qwikcity/prefetching/overview/index.mdx) permet à Qwik de donner la priorité aux parties importantes de l'interactivité avant les parties moins importantes.
...
Vous pouvez déployer votre appli Qwik comme un site statique, s'il n'y a pas besoin du SSR, grâce à notre [adaptateur SSG (Génération de site statique)](../../qwikcity/static-site-generation/overview/index.mdx).
