...
Non. En production, Qwik utilise beaucoup d'informations générées pendant le SSR (Rendu côté serveur) pour commencer la [prélecture](/qwikcity/advanced/prefetching/index.mdx) uniquement des morceaux d'interactivité disponibles sur la page courante dès que possible (ASAP). De cette façon, quand l'utilisateur clique ou interagit, le JS est déjà chargé.

## Si Qwik précharge le JS, alors quelle est la différence ?

La prélecture n'est pas la même chose que le traitement et l'exécution du JS, et Qwik n'exécute le JS que lorsque l'utilisateur interagit.

De plus, la [prélecture](/qwikcity/advanced/prefetching/index.mdx) permet à Qwik de donner la priorité aux parties importantes de l'interactivité avant les parties moins importantes.
