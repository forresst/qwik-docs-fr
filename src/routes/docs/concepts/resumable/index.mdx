...
1. Les frameworks doivent télécharger tout le code du composant associé à la page actuelle.
...
La résumabilité consiste à mettre en pause l'exécution sur le serveur et reprendre l'exécution sur le client sans avoir à rejouer et télécharger toute la logique de l'application.
...
Les frameworks existants résolvent le problème des écouteurs d'événements en téléchargeant les composants et en exécutant leurs templates pour collecter les écouteurs d'événements qui sont ensuite attachés au DOM. L'approche actuelle présente les problèmes suivants :

1. Le code du template doit être téléchargé par avance.
2. Exige que le code du template soit exécuté par avance.
3. Le code du gestionnaire d'événements doit être téléchargé par avance (pour être attaché).

L'approche ci-dessus n'est pas viable à grande échelle. Au fur et à mesure que l'application se complique, la quantité de code à télécharger par avance et à exécuter augmente proportionnellement par rapport à la taille de l'application. Cela a un impact négatif sur les performances de démarrage de l'application et donc sur l'expérience utilisateur.
...
2. Le HTML contient une URL vers le morceau et le nom du symbole. L'attribut indique à Qwikloader le morceau de code à télécharger et le symbole à récupérer dans ce morceau.
...
Les frameworks fonctionnent avec des arborescences de composants. Pour cela, les frameworks ont besoin d'une connaissance complète des arborescences des composants pour savoir quels composants doivent être rendus et quand. Si vous regardez la sortie SSR/SSG du framework existant, les informations sur les limites des composants ont été détruites. Il n'y a aucun moyen de savoir où se trouvent les limites des composants en regardant le HTML résultant. Pour recréer ces informations, les frameworks réexécutent les templates des composants et mémorisent l'emplacement des limites des composants. La réexécution constitue l'hydratation. L'hydratation est coûteuse car elle nécessite à la fois le téléchargement des templates de composants et leur exécution.
