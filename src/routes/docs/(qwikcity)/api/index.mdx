...
La fonction `action$()` est utilisée pour déclarer une nouvelle Action de Serveur dans la page/endpoint ou le layout donnée. Qwik City exécutera seulement l'action invoquée après une interaction avec l'utilisateur (comme un clic sur un bouton ou la soumission d'un formulaire).
