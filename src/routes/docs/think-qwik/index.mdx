...
La partie unique de Qwik n'est pas dans ce qu'il fait mais dans la façon dont il atteint ses objectifs. L'objectif de Qwik est d'avoir des applications instantanées, même sur les appareils mobiles. Qwik atteint cet objectif grâce à deux stratégies principales :

1. Retarder l'exécution et le téléchargement du JavaScript aussi longtemps que possible.
2. Sérialiser l'état d'exécution de l'application et du framework sur le serveur et le reprendre sur le client.

Le but de Qwik est de télécharger et exécuter que le strict minimum de l'application.
...
C'est difficile parce que nos outils ne nous aident pas à y parvenir. Presque tous nos outils résolvent les problèmes d'une manière qui rend difficile l'envoi de moins de JavaScript. Cela est dû au fait que la plupart de nos outils sont conçus pour résoudre un problème spécifique sans penser à la quantité de JavaScript qu'ils génèrent.
...
En tant qu'industrie, nous n'avons pas réfléchi à l'implication du poinds des _bundles_. Chaque outil résout individuellement un problème spécifique, mais la taille ne fait pas partie de l'équation. La taille est le problème qui émerge lorsque vous mettez tous les outils ensemble, et à ce moment-là, le développeur ne peut pas faire grand-chose.
...
Oui, Qwik nécessite une façon différente de penser et de concevoir votre application, mais la solution, c'est presque zéro JavaScript au départ avec un téléchargement progressif du JavaScript basé sur les interactions de l'utilisateur.
...
Qwik est conçu depuis le début pour produire beaucoup de zones de chargement différé. L'outillage peut décomposer votre application en plusieurs morceaux avec du chargement différé, et le runtime peut les télécharger seulement quand c'est nécessaire.
...
Parce que nous pensons qu'il existe une meilleure façon de construire des sites. Une façon qui n'implique pas des tonnes de JavaScript qui doivent être téléchargées par avance au démarrage avant que votre site ne devienne interactif. Une façon qui permet au développeur de penser à l'ajout de fonctionnalités plutôt qu'à la façon de diviser l'énorme base de code en petits morceaux. Un moyen d'avoir des sites instantanés pour une meilleure expérience utilisateur. Et tout cela, d'une manière indépendante de la taille du code de l'application.
