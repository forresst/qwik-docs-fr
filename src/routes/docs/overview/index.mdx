---
title: Vue d'ensemble
contributors:
  - adamdbradley
  - steve8708
  - manucorporat
  - gabrielgrant
---

import { getCommunity } from './community';

# Bienvenue sur Qwik

Qwik est un nouveau type de framework web qui peut fournir des applications web à chargement instantané, quelle que soit leur taille ou leur complexité. Vos sites et applis peuvent démarrer avec environ 1kb de JS (indépendamment de la complexité de l'application), et bénéficier de performances constantes à grande échelle.

[Consultez d'autres présentations, vidéos et podcasts](/media/)

## Objectifs de Qwik

<div class="card-grid">
  <div class="card">
    <h3>Usage général</h3>
    <p>Qwik peut être utilisé pour créer n'importe quel type de site web ou d'application.</p>
  </div>
    <div class="card">
    <h3>Instantané</h3>
    <p>Contrairement aux autres frameworks, Qwik est résumable, ce qui signifie que les applications Qwik ne nécessitent aucune hydratation. Cela permet aux applis Qwik d'avoir une interactivité instantanée, quelle que soit leur taille ou leur complexité.</p>
  </div>
    <div class="card">
    <h3>Optimisé pour la vitesse</h3>
    <p>Qwik a une performance sans précédent, offrant des chargements de pages complètes en moins d'une seconde même sur les appareils mobiles. Qwik atteint ce résultat en fournissant du HTML pur, et en chargeant JS de façon incrémentale, seulement quand c'est nécessaire.</p>
  </div>
</div>

## Démarrer avec Qwik

<div class="card-grid">
  <a class="card card-center" href='/docs/getting-started/'>
    <p class='icon'>🚀</p>
    <h3>npm create qwik</h3>
  </a>
  <a class="card card-center" href='/docs/think-qwik/'>
    <p class='icon'>🤔</p>
    <h3>Pourquoi Qwik ?</h3>
  </a>
  <a class="card card-center" href='/docs/components/overview/'>
    <p class='icon'>📚</p>
    <h3>API</h3>
  </a>
  <a class="card card-center" href='https://qwik.builder.io/chat'>
    <p class='icon'>{getCommunity()}</p>
    <h3>Communauté</h3>
  </a>
  <a class="card card-center" href='https://github.com/BuilderIO/qwik'>
    <p class='icon'>💙</p>
    <h3>Open source</h3>
  </a>
</div>

## La vitesse des pages est-elle vraiment importante ?

En clair, les sites lents découragent les visiteurs, ce qui coûte des millions aux entreprises. Les sites rapides ont un meilleur référencement, une meilleure interface utilisateur et sont plus rentables.

Quelques exemples tirés de [web.dev](https://web.dev/tags/case-study/) :

|                                                                                                                                                                                                                                                                                                                   |                                                                                                                                                                                                  |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Chaque 100ms de moins → 1% de conversions en plus** <br />Pour Mobify, chaque diminution de 100ms de la vitesse de chargement de la page d'accueil s'est traduite par une augmentation de 1,11% de la conversion par session, ce qui a donné une augmentation moyenne des revenus annuels de près de 380 000 $. | **50% plus rapide → 12% de ventes en plus** <br />Lorsqu'AutoAnything a réduit de moitié le temps de chargement des pages, ils ont constaté une augmentation de 12% à 13% des ventes.            |
| **20% plus rapide → 10% de conversions en plus** <br />Le détaillant Furniture Village a audité la vitesse de son site et a élaboré un plan pour résoudre les problèmes constatés, ce qui a conduit à une réduction de 20% du temps de chargement des pages et à une augmentation de 10% du taux de conversion.   | **40% plus rapide → 15% d'inscriptions en plus** <br /> Pinterest a réduit les temps d'attente perçus de 40% et cela a augmenté le trafic du moteur de recherche et les inscriptions de 15%.     |
| **850ms plus rapide → 7% de conversions en plus** <br /> COOK a réduit le temps de chargement moyen des pages de 850 millisecondes, ce qui a augmenté les conversions de 7%, diminué les taux de rebond de 7% et augmenté les pages par session de 10%.                                                           | **1 seconde de lenteur → 10 % d'utilisateurs en moins** <br />La BBC a constaté qu'elle perdait 10 % d'utilisateurs supplémentaires pour chaque seconde additionnelle de chargement de son site. |
|                                                                                                                                                                                                                                                                                                                   |                                                                                                                                                                                                  |
