...
`const store = useStore(initialStateObject)` est un hook qui crée un objet réactif. Il prend un état initial, et retourne un objet réactif.

L'objet réactif renvoyé par [`useStore()`](/docs/(qwik)/components/state/index.mdx#usestore) est comme n'importe quel autre objet, mais il est réactif. Si vous modifiez une propriété de l'objet, tout composant qui dépend de cette propriété sera mis à jour.
...
Par défaut, [`useStore()`](/docs/(qwik)/components/state/index.mdx#usestore) ne traque que les champs de premier niveau de votre store, ce qui signifie que pour que toute mise à jour soit enregistrée, vous devez mettre à jour les valeurs du champ de premier niveau.
...
Dans Qwik il y a deux façons de créer des valeurs calculées, en utilisant [`useTask$()`](/docs/(qwik)/components/lifecycle/index.mdx#usetask) ou [`useResource$()`](/docs/(qwik)/components/resource/index.mdx).

La principale différence entre les deux est que [`useTask$()`](/docs/(qwik)/components/lifecycle/index.mdx#usetask) permet les effets de bord et l'exécution est sérialisée, tandis que [`useResource$()`](/docs/(qwik)/components/resource/index.mdx) est asynchrone et plusieurs appels `useResource$()` peuvent se produire en parallèle.

`useTask$()` est généralement utilisé pour calculer l'état intermédiaire, alors que [`useResource$()`](/docs/(qwik)/components/resource/index.mdx) a une meilleure ergonomie pour calculer l'état final, utilisé pour le rendu. Voyons quelques exemples :

### Exemple de [`useTask$()`](/docs/(qwik)/components/lifecycle/index.mdx#usetask)
...
### Exemple de [`useResource$()`](/docs/(qwik)/components/resource/index.mdx)
