...
Comme toutes les méthodes `use-`, elle doit être appelée dans le contexte de [`component$()`](/docs/(qwik)/components/overview/index.mdx#component), et toutes les [règles de hook s'appliquent](/docs/(qwik)/components/lifecycle/index.mdx#use-method-rules).
...
Le callback passé à [`useResource$()`](/docs/(qwik)/components/resource/index.mdx) s'exécute juste après que le callback [`useTask$()`](/docs/(qwik)/components/lifecycle/index.mdx#usetask) se soit terminé. Veuillez vous référer à la section [Cycle de vie](../lifecycle/index.mdx) pour plus de détails.
...
- #### [`useResource$()`](/docs/(qwik)/components/resource/index.mdx) avec `<Resource />`
