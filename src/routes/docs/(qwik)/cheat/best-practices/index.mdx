...
# Éviter les balles dans le pied

## N'enregistrez pas les événements en avance avec `useVisibleTask$()`

> ⚠ Utilisez `useVisibleTask$()` avec précaution ⚠
...
Cela exige également que vous, le développeur, restiez plus attentif à la relation entre le serveur et le client (il y a une raison pour laquelle vous avez obtenu `use > Visible < Task` après tout).
...
>
> - [docs useOn / useDocument / useWindow](/docs/(qwik)/components/events/index.mdx#useon-hooks)
...
> Voir : les [docs de useLocation()](/docs/(qwikcity)/api/index.mdx#uselocation)
