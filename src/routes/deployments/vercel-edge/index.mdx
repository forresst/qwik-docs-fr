...
L'adaptateur de Qwik City pour Vercel Edge vous permet de connecter Qwik City aux [Fonctions de Vercel Edge](https://vercel.com/docs/(qwik)/concepts/functions/edge-functions).
...
Notez que vous pouvez avoir besoin d'un [compte Vercel](https://vercel.com/docs/(qwik)/concepts/get-started) afin de compléter cette étape !
...
Veuillez vous référer à la documentation de Vercel pour plus d'informations sur la façon de déployer votre site : [docs de Vercel](https://vercel.com/docs/(qwik)/concepts/deployments/overview)
...
Les [Fonctions de Vercel Edge](https://vercel.com/docs/(qwik)/concepts/functions/edge-functions) sont déployées globalement par défaut sur le réseau de Vercel Edge et vous permettent de déplacer la logique côté serveur vers le Edge, à proximité de l'origine de votre visiteur.
