...
Le `$` marque une closure comme pouvant être chargée de manière différée. Par exemple, la méthode `component$()` permet de rendre le composant chargeable de manière différée. Quand vous voyez un `$` dans le code de Qwik, vous traversez une zone de chargement différé et vous devez être attentif aux règles spéciales :
...
Dans cet exemple, les composants `<App>` et `<Greeter>` sont préparés pour vous. Le composant `<Greeter>` est déclaré à l'aide de la méthode `component$()` et est un composant Qwik. Retirez la méthode `component$()` pour convertir `<Greeter>` en un composant en ligne.
