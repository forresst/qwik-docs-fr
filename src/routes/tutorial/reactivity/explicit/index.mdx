...
En plus de la réactivité implicite créée par les templates, Qwik prend en charge l'exécution explicite de code quand une propriété change. Ceci est réalisé par le hook `useWatch$()`. Les hooks `useWatch$()` s'exécutent avant le rendu du composant et peuvent être asynchrones. Le hook peut également avoir une fonction de nettoyage qui est invoquée lors de la prochaine exécution du hook ou lorsque le composant est supprimé.
