...
Utilisez la fonction [`useResource$()`](/docs/components/resource/index.mdx) pour configurer la façon dont les données sont récupérées du serveur. Utilisez la fonction `<Resource>` pour afficher les données.
...
```jsx
  <Resource
     ...
     onResolved={(reason) => <div>Error: {reason}</div>}
