...
L'Optimiseur fait son travail en recherchant les fonctions qui se terminent par le caractère `$`. Par exemple, l'Optimiseur transformera un appel à `component$()` en un point d'entrée. Remarquez que le nom de la fonction n'a pas d'importance, mais seulement le fait qu'elle se termine par le caractère `$`.
