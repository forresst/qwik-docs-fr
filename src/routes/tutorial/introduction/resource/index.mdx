...
Qwik fournit `useResource$()` et `<Resource>` pour vous aider à récupérer et afficher des données depuis le serveur. Lorsque vous récupérez des données, l'application peut être dans l'un des trois états suivants :
...
Utilisez la fonction `useResource$()` pour configurer la façon dont les données sont récupérées du serveur. Utilisez la fonction `<Resource>` pour afficher les données.

## Récupération des données

Utilisez `useResource$()` pour configurer la façon dont les données sont récupérées du serveur.
...
La fonction `useResource$()` retourne un objet `ResourceReturn`, qui est un objet de type Promise qui peut être sérialisé par Qwik. La fonction `useResource$()` vous permet de traquer (`track`) les propriétés du store afin que la fonction `useResource$()` puisse être réactive sur les changements du store. La fonction `cleanup` vous permet d'enregistrer un code qui doit être exécuté pour libérer les ressources de l'exécution précédente. Enfin, la fonction `useResource$()` renvoie une promesse qui sera résolue avec la valeur.

## Rendu des données

Utilisez `<Resource>` pour afficher les données de la fonction `useResource$()`. La fonction `<Resource>` vous permet d'afficher un contenu différent selon si la ressource est `pending`, `resolved` ou `rejected`.
