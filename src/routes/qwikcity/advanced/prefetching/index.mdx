...
Qwik lui-même doit être configuré pour utiliser l'implémentation [prefetchEvent](../../../docs/advanced/prefetching/index.mdx) (qui est aussi la valeur par défaut de Qwik). Quand Qwik émet l'évènement [qprefetch](#prefetch-event), l'enregistrement du service worker transmet activement les données de l'évènement prefetch au service worker installé et actif.
...
Pour en savoir plus sur la [Mise en cache des paires de requêtes et de réponses](/qwikcity/advanced/prefetching/index.mdx#caching-request-and-response-pairs).

De plus, le service worker s'assure que plusieurs requêtes pour le même bundle ne se produisent pas en même temps. Pour en savoir plus, consultez la documentation [Parallélisation des requêtes réseau](#parallelizing-network-requests).
...
D'autres avantages incluent la [Parallélisation des requêtes réseau](#parallelizing-network-requests).
