...
Les menus sont des données contextuelles déclarées avec des fichiers `menu.md`. Consultez la [définition du fichier menu](/qwikcity/advanced/menu/index.mdx) pour plus d'informations sur le format et l'emplacement du fichier.
...
`useLocation()` encode les [Paramètres de la route](/qwikcity/routing/index.mdx) en tant que paramètres.
...
Veuillez consulter la section [Chargeur de route](/qwikcity/route-loader/index.mdx) pour plus d'informations.

## `action$()`

La fonction `action$()` est utilisée pour déclarer une nouvelle Action de Serveur dans la page/endpoint ou le layout donnée. QwikCity exécutera seulement l'action invoquée après une interaction avec l'utilisateur (comme un clic sur un bouton ou la soumission d'un formulaire).

Veuillez vous référer à la section [Actions de Serveur](/qwikcity/action/index.mdx) pour plus d'informations.
...
Le composant `Form` est une enveloppe autour de l'élément natif `<form>`, et il est conçu pour fonctionner en parallèle avec les [Actions du serveur](/qwikcity/action/index.mdx).
