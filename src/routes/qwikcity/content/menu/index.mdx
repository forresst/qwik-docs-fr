...
2. Utilisation de la fonction `useContent()` pour récupérer la structure du menu dans un modèle pour le rendu. [Lire la suite ici](../../../qwikcity/api/use-content/index.mdx)
...
Au moment de l'exécution, tout composant peut récupérer le menu avec le hook `useContent()`. Le type retourné est `ContentMenu`.
...
Bien que `useContent()` peut être invoqué à partir de n'importe quel composant faisant partie de la route actuelle. Il est généralement utilisé dans un composant de mise en page (ou un composant utilisé par la mise en page) pour rendre le menu. Un exemple d'utilisation est présenté ici :
