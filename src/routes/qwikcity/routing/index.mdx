...
> Le dossier `[skuId]` peut être n'importe lequel des milliers de produits que vous avez dans votre base de données. Il serait peu pratique de créer une route pour chaque produit. Au lieu de cela, nous devons définir un paramètre de route (une partie de l'URL) qui sera utilisé pour extraire le `[skuId]`.
