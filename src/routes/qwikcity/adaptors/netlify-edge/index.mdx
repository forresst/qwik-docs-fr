...
Les [Déclarations des fonctions Netlify Edge](https://docs.netlify.com/edge-functions/declarations/) 
peuvent être configurées pour s'exécuter sur des modèles d'URL spécifiques. Chaque déclaration de fonction edge associe
un modèle de chemin d'accès au site à une fonction à exécuter sur les requêtes qui correspondent à ce chemin. Une seule requête peut exécuter une chaîne de fonctions edge à partir d'une série de déclarations. Une même fonction edge peut être associée à plusieurs chemins d'accès à travers plusieurs déclarations.
