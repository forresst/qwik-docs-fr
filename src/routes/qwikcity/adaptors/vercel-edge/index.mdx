...
De plus, dans le `package.json`, le script `build.server` sera mis à jour avec le build de Vercel Edge.
...
Notez que vous pouvez avoir besoin d'un [compte Vercel](https://docs.Vercel.com/get-started/) afin de compléter cette étape !

## Déploiement de la production

Après avoir installé l'intégration en utilisant `npm run qwik add`. Le projet est prêt à être déployé sur Vercel. Cependant, vous devrez créer un dépôt git et y pousser le code.
