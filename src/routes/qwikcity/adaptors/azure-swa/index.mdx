...
L'adaptateur ajoutera un nouveau `vite.config.ts` dans le répertoire `adaptors/`, et un nouveau fichier d'entrée sera créé, tel que :
...
De plus, dans le `package.json`, le script `build.server` sera mis à jour avec le build de Azure Static Web Apps.
...
Après avoir installé l'intégration en utilisant `npm run qwik add`. Le projet est prêt à être déployé sur Azure Static Web Apps.
...
2. Déployez depuis GitHub
   ...
   Lors de la configuration d'une Azure Static Webapp via le portail Azure, vous pouvez choisir le dépôt GitHub à déployer.
